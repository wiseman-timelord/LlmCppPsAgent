...
3) Menu to select prompt format, dunno, maybe compare model name to shortlist, to see if previously set, and if not, then ask user to select from options, "### Instruction" (Daydreamer v3) or "[INST] <<SYS>>" (Llama2) or "{system_message} User:", syntax formats, will review this later.
