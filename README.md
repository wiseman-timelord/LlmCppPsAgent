...
3) Reporting of temps on, CPU & GPU, each loop through LibreOpenHardwareMonitor, this will allow users to notice if there are issues with their cooling setup, and maybe even some kind of emergency exit if the cpu/gpu goes over 80c, as around there will sometimes crash, and little over that could damage hardware, of course with --temps flag to ignore over-temp.
